    ...
    <List class="leaveContent">
      ...
      >
        ...
        >
          <div class="head">
            <p
              v-show="publishURL == '/note/accessPulish'"
              class="number_id"
            >{{index+1}} 楼</p>
            <p
              v-show="publishURL == '/message/leavemessage'"
              class="number_id"
            >{{item.id}} 楼</p>
            <img :src="item.Imgsrc">
            <a>{{item.name}}<span
                v-show="item.username === 'cosyer'"
                class="chief"
              >站长</span></a>
          </div>
        ...
        >
          ...
          >
            ...
            >
              ...
              <span
                v-show="replyitem.username === 'cosyer'"
                class="chief"
              >站长</span>
            </p>
            <p
              @click="SetPaddingReply(replyitem, item,  j)"
              class="reply"
            >回复</p>
          </ListItem>
        </div>
...
export default {
  ...
  methods: {
    ...
    handelInputContent() {
      ...
      if (!this.replyValue)
        return this.$Message.error("内容不能为空呀,请输入内容！");
    ...
    publish() {
      if (localStorage.getItem("username")) {
        ...
        if (this.value) {
          ...
          /* leave message end */
          PostMessage(this.publishURL, this.obj).then((res) => {
            if (res.data.err == 0) {
              this.$Message.success(res.data.message);
              setTimeout(() => {
                location.reload();
        ...
        this.$Message.error("请先去登录再来留言哦,(ノへ￣、)！");
      }
    },
  },
};
