<template>
  <div class="myInfo">
    <div
      v-if="!username"
      style="text-align:center;padding-top:8rem;"
    >
      糟糕，您还没有登录检测不到信息! ~§(*￣▽￣*)§~
    </div>
    <div
  ...
  methods: {
        ...
        PostMessage("/user/getuserInfo", { token: this.username }).then(
          (res) => {
            if (res.data.err === 0) {
              ...
              this.$Message.error("对不起,您的登录信息已过期请重新登录。");
    ...
    primaryInfo() {
      ...
      if (this.MyInfo.name.length > 12)
        return this.$Message.error("昵称长度不得大于8位");
